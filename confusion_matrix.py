...
def confusion_matrix_per_image(label_id_map, confusion_matrix, ground_truth_arr, preds_arr, iou_threshold):
    ...
    for pred_idx, pred_shape in enumerate(preds_arr):
        ...
        for gt_idx, gt_shape in enumerate(ground_truth_arr):
            if pred_shape.seen == 1:  # one prediction can only belong to one ground truth
                break
            elif iou_matrix[pred_idx, gt_idx] >= iou_threshold:
                ...
                pred_shape.coord = [gt_label_id, pred_label_id]  # store this coord in confusion matrix to be queried later
    ...
    for num_tp, gt_label, in zip(true_positives, gt_labels):
        if num_tp == 0:  # FN -> model cannot predict ground truth (ground truth but no prediction)
